...
function isAvailableNow(bookings) {
  const now = new Date()
  return !bookings.some((booking) => overlaps(new Date(booking.startsAt), new Date(booking.endsAt), now, now))
}
...
export async function getCourtsWithAvailability() {
  ...
  return Promise.all(
    courts.map(async (court) => {
      ...
      const availableNow = isAvailableNow(bookings)

      return {
        ...court,
        availableNow,
        availabilityLabel: availableNow ? 'متاح الآن' : 'محجوز جزئيًا',
      }
    })
  )
}
