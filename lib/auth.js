...
export async function getUserFromSessionToken(token) {
  ...
  if (prisma) {
    ...
    return user && user.isActive !== false ? { ...user, role: user.role } : null
  }
...
export async function getUserByPhone(phone) {
  const prisma = getPrisma()
  if (prisma) {
    return prisma.user.findFirst({ where: { phone, isActive: true } })
  }
  return memoryStore.users.find((user) => user.phone === phone && user.isActive !== false) || null
}
...
export async function upsertUser({ name, phone, role }) {
  ...
  if (prisma) {
    return prisma.user.upsert({
      ...
      create: { name, phone, role, isActive: true },
    })
  }
...
export async function findUserById(id) {
  ...
  return memoryStore.users.find((user) => user.id === id && user.isActive !== false) || null
}
